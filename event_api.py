...
class ConnectionManager:
    """Manages active WebSocket connections keyed by thread_id."""
    ...
    async def send(self, thread_id: str, data: dict) -> bool:
        """Send event to connected client. Returns False if no connection."""
...
# ─────────────────────────────────────────────────────────────────────────────
# Per-pipeline event state
# ─────────────────────────────────────────────────────────────────────────────

# Queue of WebSocket events per thread — consumed by the WS handler
pipeline_queues:        Dict[str, asyncio.Queue]  = {}
# Signals the background runner to resume after a HITL decision
pipeline_resume_events: Dict[str, asyncio.Event]  = {}
# Background asyncio tasks (one per pipeline run)
pipeline_tasks:         Dict[str, asyncio.Task]   = {}
# Lightweight metadata (status, paused_at) for REST polling
...
def get_gate_context(state_values: dict, gate: str) -> dict:
    """Returns the minimal context a HITL reviewer needs for each gate."""
    if gate == "hitl_gate_1":
        return {
...
def get_node_summary(node_name: str, node_output: dict) -> dict:
    """
    Extracts a compact summary from a node's state-update output so that
    WebSocket events stay lightweight (no full code blobs).
    """
    try:
        extractors = {
            "intent_agent":         lambda o: {
                "app_type": o.get("intent_manifest", {}).get("app_type"),
                "modules":  len(o.get("intent_manifest", {}).get("modules", [])),
            },
            "ip_guard_agent":       lambda o: {
                "overall_risk": o.get("ip_clearance", {}).get("overall_risk"),
                "flagged":      len(o.get("ip_clearance", {}).get("flagged_items", [])),
            },
            "compliance_agent":     lambda o: {
                "frameworks": [
                    f["name"]
                    for f in o.get("compliance_rules", {}).get("applicable_frameworks", [])
                ],
                "risk": o.get("compliance_rules", {}).get("overall_compliance_risk"),
            },
            "architecture_agent":   lambda o: {
                "pattern": o.get("architecture", {}).get("selected_pattern"),
                "layers":  len(o.get("architecture", {}).get("layers", [])),
            },
            "codegen_agent":        lambda o: {
                "files": [
                    m["filename"]
                    for m in o.get("generated_code", {}).get("modules", [])
                ],
            },
            "optimizer_agent":      lambda o: {
                "optimizations": sum(
                    len(m.get("optimizations_applied", []))
                    for m in o.get("generated_code", {}).get("modules", [])
                ),
            },
            "security_agent":       lambda o: {
                "risk":     o.get("security_report", {}).get("overall_security_risk"),
                "findings": len(o.get("security_report", {}).get("findings", [])),
                "passed":   o.get("security_report", {}).get("passed"),
            },
            "explainability_agent": lambda o: {
                "decisions": len(o.get("explainability_docs", {}).get("decision_log", [])),
                "modules":   len(o.get("explainability_docs", {}).get("module_explanations", [])),
            },
            "quality_agent":        lambda o: {
                "score":  o.get("quality_report", {}).get("overall_quality_score"),
                "passed": o.get("quality_report", {}).get("passed"),
            },
            "audit_agent":          lambda o: {
                "final_status": (o.get("quality_report") or {})
                    .get("final_audit", {})
                    .get("final_status"),
            },
        }
...
# ─────────────────────────────────────────────────────────────────────────────
# Knowledge / rule tables (unchanged)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Agent functions (unchanged logic)
# ─────────────────────────────────────────────────────────────────────────────

def intent_agent(state: DevState) -> dict:
    print("[intent_agent] Starting")
    response = llm.invoke(
        f"You are an AI software architect. Convert the user description into a structured intent manifest. "
        f"Respond ONLY with valid JSON.\nInput: {state['raw_input']}\nSchema: {INTENT_SCHEMA}"
    )
    manifest = extract_json(response.text)
    print(f"[intent_agent] Done — app_type: {manifest.get('app_type')}")
    return {
...
{CODEGEN_SCHEMA}""")
    ...
    print(f"[codegen_agent] Done — {len(modules)} files generated")
    return {
        "generated_code": code,
...
{OPTIMIZER_SCHEMA}""")
    ...
    print(f"[optimizer_agent] Done — {len(opts.get('optimizations', []))} optimizations")
    return {
        "generated_code": {**code, "modules": updated_modules},
        "audit_log": [make_audit_entry("optimizer_agent", f"Optimization — {len(opts.get('optimizations', []))} improvements", {"optimizations": len(opts.get("optimizations", [])), "files_rewritten": list(rewritten_map.keys())})],
    }
...
def run_local_security_scan(modules: list, ip_clearance: dict) -> list:
    ...
    stdlib = {
        "os","sys","re","json","datetime","typing","pathlib","hashlib","hmac","secrets",
        "logging","functools","itertools","collections","abc","enum","pydantic","starlette",
        "asyncpg","sqlalchemy","alembic","pytest","httpx","uvicorn","fastapi",
    }
    findings = []
    for module in modules:
        ...
        for rule_name, rule in SECURITY_RULES.items():
            ...
            if matches:
                findings.append({
                    "filename": filename, "rule": rule_name, "severity": rule["severity"],
                    "owasp_ref": rule["owasp"], "line_hint": str(matches[0])[:120],
                    "fix": rule["fix"], "source": "local_scan",
                })
        for imp in re.findall(r'^(?:import|from)\s+(\w+)', code, re.MULTILINE):
            if imp.lower() not in stdlib and imp.lower() not in cleared_libs:
                findings.append({
                    "filename": filename, "rule": "unlicensed_import", "severity": "high",
                    "owasp_ref": "IP Compliance", "line_hint": f"import {imp}",
                    "fix": f"'{imp}' not IP-cleared", "source": "local_scan",
                })
    return findings
...
def check_compliance_tag_coverage(modules: list) -> dict:
    ...
    return {
        ...
        "coverage_percent":   round((len(files_with) / total * 100) if total > 0 else 0, 1),
    }
...
def security_agent(state: DevState) -> dict:
    ...
    print(f"[security_agent] Done — risk: {report.get('overall_security_risk')}, passed: {report['passed']}")
    return {
...
def route_after_hitl_1(state: DevState) -> str:
    ...
    result = "compliance_agent" if (gate1 and gate1[-1]["choice"] in ("A", "M")) else "intent_agent"
...
# ─────────────────────────────────────────────────────────────────────────────
# Event-driven pipeline background runner
# ─────────────────────────────────────────────────────────────────────────────

async def run_pipeline_background(thread_id: str, initial_state: dict) -> None:
    """
    Runs the full pipeline lifecycle in a background asyncio Task.

    Flow:
      1. Streams pipeline segments, emitting a `node_completed` WS event per node.
      2. On each HITL interrupt, emits `hitl_pause` and suspends via asyncio.Event.
      3. The `/decide` REST endpoint updates LangGraph state and sets the Event,
         which resumes this coroutine for the next segment.
      4. Emits `pipeline_complete` (or `error`) as the final event.

    All WS events are placed on `pipeline_queues[thread_id]`.
    """
    ...
    def _emit_sync(event: dict) -> None:
        """Thread-safe event push — called from inside asyncio.to_thread."""
        loop.call_soon_threadsafe(queue.put_nowait, event)

    def _now() -> str:
        return datetime.now(timezone.utc).isoformat()

    input_state: Any = initial_state   # first segment uses initial state; subsequent use None (checkpoint)

    try:
        while True:
            # ── Stream one pipeline segment (synchronous LangGraph call → thread) ──
            ...
            if not next_nodes:
                ...
                pipeline_meta[thread_id].update({
                    "status":       "complete",
                    "final_status": final_audit.get("final_status"),
                })
                break

            # ── Paused at a HITL gate ─────────────────────────────────────────
            ...
            # ── Wait for the /decide endpoint to signal resume ────────────────
            ...
            input_state = None   # resume from LangGraph checkpoint
...
async def pipeline_websocket(websocket: WebSocket, thread_id: str):
    ...
    Event types pushed to the client:
      • node_completed  — a pipeline node finished; includes a compact summary
      • hitl_pause      — pipeline is waiting for a human decision at a gate
      • pipeline_complete — final state (successful finish)
      • error           — unrecoverable failure
      • ping            — keepalive (no action needed)

    The connection is held open until `pipeline_complete` or `error` arrives.
    HITL decisions are submitted via POST /pipeline/{thread_id}/decide (REST).
    """
    if thread_id not in pipeline_queues:
        await websocket.accept()
        await websocket.send_json({
            "type":    "error",
            "message": f"Pipeline '{thread_id}' not found. Call POST /pipeline/start first.",
        })
    ...
    try:
        while True:
            try:
                # Wait up to 60 s for the next event; send a keepalive if idle
            ...
            # Terminal events — close after a short grace period
...
async def start_pipeline(request: RunRequest):
    ...
    Returns immediately with a `thread_id` and `ws_url`.
    Connect to the WebSocket URL to receive real-time events.
    The pipeline begins running in the background straight away.
    """
    thread_id = str(uuid4())
    config    = get_config(thread_id)
...
async def hitl_decide(thread_id: str, request: HITLDecisionRequest):
    """
    Submit a Human-in-the-Loop decision at a paused gate.

    choice values:
      A — Approve (proceed to next stage)
      R — Reject  (re-run previous stage)
      M — Modify  (approve with notes; proceed)
      H — Hold    (terminal hold; pipeline ends)

    The pipeline resumes immediately after the decision is recorded.
    Progress continues to be streamed over the existing WebSocket.
    """
    config  = get_config(thread_id)
    ...
    # Determine current gate from LangGraph state
    current    = await asyncio.to_thread(pipeline_graph.get_state, config)
    next_nodes = list(current.next)
    ...
    if request.choice not in ("A", "R", "M", "H"):
        raise HTTPException(status_code=422, detail="choice must be one of: A (approve), R (reject), M (modify), H (hold)")

    print(f"[{paused_at}] Decision received — choice: {request.choice}, approver: {request.approver}")
    ...
    # Persist decision into LangGraph checkpoint
    ...
    if resume_evt:
        ...
        # Runner hasn't reached the wait yet — it will pick up state from checkpoint
        print(f"[{paused_at}] No resume event found — pipeline will read state from checkpoint on next iteration")

    pipeline_meta[thread_id]["status"] = "running"
    ...
    return {
        ...
        "message":   "Pipeline resuming. Continue watching the WebSocket for progress.",
    }
...
async def get_pipeline_state(thread_id: str):
    """
    Lightweight REST poll for pipeline status.
    Prefer the WebSocket for real-time updates; use this for reconnect / status checks.
    """
    if thread_id not in pipeline_queues:
        raise HTTPException(status_code=404, detail=f"Pipeline not found: {thread_id}")
    ...
    return {
        ...
        "final_status":     final_audit.get("final_status"),
        "audit_log":        state_vals.get("audit_log", []),
        "hitl_decisions":   state_vals.get("hitl_decisions", []),
        "immutable_digest": final_audit.get("immutable_digest"),
        "ws_url":           f"/ws/pipeline/{thread_id}",
    }
...
async def get_pipeline_result(thread_id: str):
    """
    Full final output once the pipeline has completed.
    Returns 400 if the pipeline is still running or paused.
    """
    if thread_id not in pipeline_queues:
        raise HTTPException(status_code=404, detail=f"Pipeline not found: {thread_id}")
    ...
    if meta.get("status") not in ("complete", "error"):
        raise HTTPException(
            status_code=400,
            detail=f"Pipeline is not finished yet — current status: {meta.get('status')}. "
                   "Subscribe to the WebSocket or poll /state.",
        )
    ...
    return {
        "thread_id":           thread_id,
        "final_status":        final_audit.get("final_status"),
        "intent_manifest":     state_vals.get("intent_manifest"),
        "compliance_rules":    state_vals.get("compliance_rules"),
        "ip_clearance":        state_vals.get("ip_clearance"),
        "architecture":        state_vals.get("architecture"),
        "generated_code":      state_vals.get("generated_code"),
        "explainability_docs": state_vals.get("explainability_docs"),
        "security_report":     state_vals.get("security_report"),
        "quality_report":      state_vals.get("quality_report"),
        "hitl_decisions":      state_vals.get("hitl_decisions"),
        "audit_log":           state_vals.get("audit_log"),
        "immutable_digest":    final_audit.get("immutable_digest"),
        "sign_off_note":       final_audit.get("sign_off_note"),
